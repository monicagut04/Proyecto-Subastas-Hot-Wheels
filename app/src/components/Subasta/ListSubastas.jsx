...
export function ListSubastas() {
    ...
    const [canceladas, setCanceladas] = useState([]); // NUEVO ESTADO
    ...
    useEffect(() => {
        const fetchSubastas = async () => {
            try {
                const [resActivas,resBorradores,resFinalizadas , resCanceladas] = await Promise.all([
                    SubastaService.getActivas(),
                    SubastaService.getBorradores(),
                    SubastaService.getFinalizadas(),
                    SubastaService.getCanceladas()
                ]);
                ...
                if (!resBorradores.data.success) throw new Error(resFinalizadas.data.message);
                if (!resCanceladas.data.success) throw new Error(resFinalizadas.data.message);


                setActivas(resActivas.data.data || []);
                setBorradores(resBorradores.data.data || []);
                setFinalizadas(resFinalizadas.data.data || []);
                setCanceladas(resCanceladas.data.data || []);
            } catch (err) {
    ...
    if (error) return (
        <div className="mx-auto max-w-3xl mt-10 p-6 bg-zinc-900 border border-red-900/50 rounded-2xl flex items-center gap-4 text-red-400 shadow-2xl">
            ...
            <p className="text-lg font-bold italic uppercase tracking-tighter">Error de Radar: {error}</p>
        </div>
    );

const subastasAMostrar = vistaActual === 'activas' ? activas : 
                                            vistaActual === 'borradores' ? borradores : 
                                            vistaActual === 'finalizadas' ? finalizadas : 
                                            canceladas;
    return (
        <div className="min-h-screen bg-black text-zinc-100 p-4 md:p-8">
            <div className="mx-auto max-w-7xl">
                ...
                <div className="mb-10 space-y-2">
                    ...
                    <Link 
                        to="/subasta/create" 
                        className="bg-red-600 hover:bg-red-700 text-white px-6 py-4 rounded-2xl flex items-center gap-3 font-black uppercase tracking-widest text-sm transition-all shadow-[0_0_20px_rgba(220,38,38,0.3)] hover:scale-105 border border-red-500/50"
                    >
                        <span className="text-xl leading-none">+</span> NUEVA SUBASTA
                    </Link>
                </div>

                {/* Selector de Vista Estilo Dashboard */}
                <div className="flex p-1.5 bg-zinc-900/50 border border-zinc-800 rounded-2xl w-fit mb-12 backdrop-blur-sm overflow-x-auto">
                    <button 
                        onClick={() => setVistaActual('activas')}
                        className={`flex items-center gap-3 px-8 py-3 rounded-xl font-black uppercase italic tracking-widest text-xs transition-all duration-300 ${vistaActual === 'activas' ? 'bg-green-600 text-white shadow-[0_0_20px_rgba(220,38,38,0.4)]' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800'}`}
                    >
                        <TrendingUp className="h-4 w-4" /> ACTIVAS ({activas.length})
                    </button>
                    <button 
                        onClick={() => setVistaActual('borradores')}
                        className={`flex items-center gap-3 px-8 py-3 rounded-xl font-black uppercase italic tracking-widest text-xs transition-all duration-300 ${vistaActual === 'borradores' ? 'bg-yellow-600 text-white shadow-[0_0_20px_rgba(202,138,4,0.4)]' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800'}`}
                    >
                        <Clock className="h-4 w-4" /> Para publicar ({borradores.length})
                    </button>
                    <button 
                        onClick={() => setVistaActual('finalizadas')}
                        className={`flex items-center gap-3 px-8 py-3 rounded-xl font-black uppercase italic tracking-widest text-xs transition-all duration-300 ${vistaActual === 'finalizadas' ? 'bg-zinc-700 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800'}`}
                    >
                        <History className="h-4 w-4" /> FINALIZADAS ({finalizadas.length})
                    </button>
                    <button 
                        onClick={() => setVistaActual('canceladas')}
                        className={`flex items-center gap-3 px-8 py-3 rounded-xl font-black uppercase italic tracking-widest text-xs transition-all duration-300 ${vistaActual === 'canceladas' ? 'bg-red-900 text-white shadow-[0_0_20px_rgba(127,29,29,0.4)]' : 'text-zinc-500 hover:text-zinc-200 hover:bg-zinc-800'}`}
                    >
                        <AlertCircle className="h-4 w-4" /> CANCELADAS ({canceladas.length})
                    </button>
                </div>
                        ...
                        {subastasAMostrar.map((subasta) => (
                            <div key={subasta.id_subasta} className="group relative bg-zinc-900/40 border border-zinc-800 rounded-[2rem] p-8 shadow-2xl hover:border-red-600/50 transition-all duration-500 flex flex-col justify-between overflow-hidden">
                                ...
                                <Link to={`/subasta/detail/${subasta.id_subasta}`} className="relative z-10">
                                    ...
                                    {vistaActual === 'borradores' ? 'Preparar' : 'Entrar a la sala'} <ChevronRight className="h-4 w-4" />                                    </button>
                                </Link>
                            </div>
                        ))}
                    </div>
